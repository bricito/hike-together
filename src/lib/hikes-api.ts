...
export async function fetchMyParticipation(
  hikeId: string,
  userId: string,
): Promise<{ id: string; status: ParticipantStatus; payment_status: PaymentStatus | null } | null> {
  const { data, error } = await supabase
...
export async function requestToJoinHike(hikeId: string) {
  ...
  if (hike) {
    ...
    // Notification push FCM à l'organisateur
    const { error: fcmError } = await supabase.functions.invoke(
      "send-fcm-notification",
      {
        body: {
          user_id: hike.organizer_id, // ✅ l'organisateur, pas le participant
          title: "Nouvelle demande de participation",
          body: `${profile?.full_name ?? "Quelqu'un"} souhaite rejoindre votre randonnée`,
