...
export const Route = createFileRoute("/hikes/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!h) {
      return {
        meta: [{ title: "Randonnée — BlablaHike" }],
      };
    }
    ...
    return {
      meta: [
        ...
        {
          name: "description",
          content: `${h.title} à ${h.location}. Randonnée ${h.difficulty}, ${h.durationHours}h, ${h.elevationM}m de dénivelé.`,
        },
        {
          property: "og:title",
          content: h.title,
        ...
        {isPaid && totalCents != null && totalCents > 0 && (
          <div className="rounded-2xl bg-secondary/50 border border-border p-3 mb-4 text-sm text-muted-foreground">
            ...
            <p className="text-xs mt-1">
              Frais de service BlablaHike + frais Stripe inclus.
            </p>
          </div>
        )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-3xl shadow-[var(--shadow-elegant)] max-w-md w-full p-6">
        ...
        <p className="text-sm text-muted-foreground mb-2">
          Êtes-vous sûr de vouloir annuler votre participation à cette
          randonnée ?
        </p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-3xl shadow-[var(--shadow-elegant)] max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <Field label="Équipement (séparé par des virgules)">
            <Input
...
function HikeDetail() {
  ...
  const canCancelParticipation =
    new Date(hike.starts_at).getTime() -
      Date.now() >
    24 * 60 * 60 * 1000;
  ...
  const joinMut = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        "Demande envoyée ! L'organisateur va l'examiner."
      );
    },
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-6 mt-6 text-sm">
            ...
            <Stat
              icon={Clock}
              label="Durée"
              value={`${hike.durationHours}h`}
            />

            <Stat
              icon={TrendingUp}
              label="Dénivelé"
              value={`${hike.elevationM}m`}
            />

            <Stat
              icon={Users}
              label="Groupe"
              value={`${hike.maxParticipants - hike.spotsLeft}/${hike.maxParticipants}`}
            />
          </div>
          ...
          <div className="mt-10 flex items-center gap-4 p-5 rounded-3xl bg-card shadow-[var(--shadow-soft)]">
            <Link
              to="/profile/$id"
              params={{
                id: hike.organizer.id,
              }}
            >
              <img
                src={hike.organizer.avatar}
                alt={hike.organizer.name}
                className="h-14 w-14 rounded-full object-cover hover:ring-2 hover:ring-primary transition-all"
              />
            </Link>
            ...
            <div className="flex items-start gap-3 p-5 rounded-3xl bg-card border border-border shadow-[var(--shadow-soft)]">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Soyez à l'heure ! Contactez
                  l'organisateur en cas de problème.
                </p>
              </div>
            </div>
          </div>
            ...
            {isOrganizer ? (
              ...
              <>
                ...
                <p className="text-[11px] text-muted-foreground text-center mt-3">
                  Un compte est nécessaire pour rejoindre
                </p>
              </>
                      ...
                      >
                        ...
                        >
                          <img
                            ...
                            alt=""
                            className="h-9 w-9 rounded-full object-cover hover:ring-2 hover:ring-primary transition-all"
                          />
                        </Link>
