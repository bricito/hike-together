...
function MyProfilePage() {
  ...
  const { data: profile, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select(
          "id, full_name, avatar_url, bio, city, country, hiking_level"
        )
        .eq("id", user!.id)
        .maybeSingle();
  ...
  const handlePhotoUpload = async (
    ...
    try {
      ...
      if (!blob) {
        throw new Error("Compression échouée.");
      }

      // Nouveau fichier compressé
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (trimmed.length > 60) {
        throw new Error("Pseudo trop long (max 60)");
      }

      if (bio.length > 1000) {
        throw new Error("Biographie trop longue (max 1000)");
      }
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Toutes les images sont automatiquement
                        compressées.
                      </p>
                    </div>
