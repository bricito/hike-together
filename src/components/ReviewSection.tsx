...
// ---------------------------------------------------------------------------
// Single review card (collapsed / expanded)
// ---------------------------------------------------------------------------
function ReviewCard({ target }: { target: PendingReviewTarget }) {
  ...
  const handleSubmit = () => {
    ...
    mutate({
      hike_id: target.hikeId,
      reviewed_user_id: target.targetUserId,
      role: target.role,
      rating,
      badge_punctual: punctual,
  ...
  const initials = target.targetName
    .split(" ")
    .map((w) => w[0])
    .join("")
    .toUpperCase()
  ...
  return (
    <div className="rounded-2xl border bg-card overflow-hidden">
      ...
      <button
        type="button"
        onClick={() => !target.alreadyReviewed && windowStatus === "open" && setOpen((o) => !o)}
          ...
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-2">
              <BadgePill
                ...
                label="Ponctuel"
                active={punctual}
                onToggle={() => setPunctual((v) => !v)}
              />
              <BadgePill
                emoji="😊"
                label="Sympa"
                active={friendly}
                onToggle={() => setFriendly((v) => !v)}
              />
              <BadgePill
                emoji="🦺"
                label="Prudent"
                active={safe}
                onToggle={() => setSafe((v) => !v)}
              />
            </div>
          </div>
    ...
    <section className="mt-10 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Notez vos compagnons de randonnée après chaque sortie.
        </p>
      </div>
      ...
      {pending.length > 0 && (
        <div className="space-y-3">
          {pending.map((t) => (
            <ReviewCard key={`${t.hikeId}-${t.targetUserId}`} target={t} />
          ))}
        </div>
      )}
